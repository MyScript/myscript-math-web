...
suite('<iron-query-params>', function () {
  ...
  test('basic functionality with ?key=value params', function () {
    ...
    expect(paramsElem.paramsObject).to.deep.eq({}); // Check the mapping from paramsString to paramsObject.
    ...
    expect(paramsElem.paramsString).to.be.equal('greeting=hello&target=world&another=key');
  });
  test('encoding of params', function () {
    var mappings = [{
      ...
      object: {
        'yes, ok? what is up with ˚thiiis˚': '☃'
      }
    }];
    mappings.forEach(function (mapping) {
      // Clear
      paramsElem.paramsObject = {};
      expect(paramsElem.paramsString).to.be.equal(''); // Test going from string to object

      paramsElem.paramsString = mapping.string;
      expect(paramsElem.paramsObject).to.deep.equal(mapping.object); // Clear again.

      paramsElem.paramsObject = {};
      expect(paramsElem.paramsString).to.be.equal(''); // Test going from object to string
