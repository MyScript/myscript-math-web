...
<body>
  ...
  <script type="module">import './test-overlay.js';
...
import { dom } from "../../polymer/lib/legacy/polymer.dom.js"; // Need to discover if html or body is scrollable.
...
suite('scroll actions in shadow root', function () {
  ...
  test('cancel scrollAction closes the overlay on scroll outside', function (done) {
    ...
    runAfterOpen(overlay, function () {
      overlay.addEventListener('iron-overlay-canceled', function (event) {
        assert.equal(event.detail.type, 'scroll', 'detail contains original event'); // In Polymer 1.x with dom=shadow, triggering a scroll event on a
