...
--><html><head>
  ...
  <script type="module" src="../iron-scroll-manager.js"></script>
  <script type="module" src="./x-scrollable-element.js"></script>
</head>
...
<body>
  ...
  <script type="module">import { pushScrollLock, removeScrollLock, elementIsScrollLocked } from '../iron-scroll-manager.js';
...
suite('IronScrollManager', function () {
  ...
  suite('constraining scroll in the DOM', function () {
    ...
    test('recognizes sibling as locked', function () {
      expect(elementIsScrollLocked(childTwo)).to.be.equal(true);
    });
    test('recognizes neice as locked', function () {
      expect(elementIsScrollLocked(grandChildTwo)).to.be.equal(true);
    });
    test('recognizes parent as locked', function () {
      expect(elementIsScrollLocked(parent)).to.be.equal(true);
    });
    test('recognizes ancestor as locked', function () {
      expect(elementIsScrollLocked(ancestor)).to.be.equal(true);
    });
    test('recognizes locking child as unlocked', function () {
      expect(elementIsScrollLocked(childOne)).to.be.equal(false);
    });
    test('recognizes descendant of locking child as unlocked', function () {
      expect(elementIsScrollLocked(grandChildOne)).to.be.equal(false);
    });
    test('unlocks locked elements when there are no locking elements', function () {
      removeScrollLock(childOne);
      expect(elementIsScrollLocked(parent)).to.be.equal(false);
    });
