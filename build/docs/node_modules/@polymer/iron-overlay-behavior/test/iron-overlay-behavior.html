...
--><html><head>
  ...
  <script type="module">import '@polymer/iron-test-helpers/iron-test-helpers.js';
...
suite('basic overlay', function () {
  ...
  test('_renderOpened called only after is attached', function (done) {
    var overlay = document.createElement('test-overlay'); // The overlay is ready at this point, but not yet attached.

    var spy = sinon.spy(overlay, '_renderOpened'); // This triggers _openedChanged.

    overlay.opened = true; // Wait long enough for requestAnimationFrame callback.

    overlay.async(function () {
      assert.isFalse(spy.called, '_renderOpened not called'); // Because not attached yet, overlay should not be the current overlay!
  ...
  test('open() triggers iron-resize', function (done) {
    var callCount = 0; // Ignore iron-resize triggered by window resize.
  ...
  test('closed overlay does not trigger iron-resize when its content changes', function (done) {
    ...
    dom(overlay).appendChild(document.createElement('div')); // Wait for MutationObserver to be executed.
  ...
  test('close an overlay quickly after open', function (done) {
    ...
    overlay.async(function () {
      ...
      this.close(); // wait for any exceptions to be thrown until the transition is done
  ...
  test('open overlay on mousedown does not close it', function (done) {
    ...
    document.body.appendChild(btn); // It triggers mousedown, mouseup, and click.
...
suite('focus handling', function () {
  ...
  test('node with autofocus is focused', function (done) {
    runAfterOpen(overlay, function () {
      assert.equal(dom(overlay).querySelector('[autofocus]'), document.activeElement, '<button autofocus> is focused');
      done();
    });
  });
  test('no-auto-focus will not focus node with autofocus', function (done) {
    ...
    runAfterOpen(overlay, function () {
      assert.notEqual(dom(overlay).querySelector('[autofocus]'), document.activeElement, '<button autofocus> not focused after opened');
      done();
    ...
    assert.notEqual(dom(overlay).querySelector('[autofocus]'), document.activeElement, '<button autofocus> not immediately focused');
  });
  ...
  test('overlay with-backdrop and 1 focusable: prevent TAB and trap the focus', function (done) {
    ...
    runAfterOpen(overlay, function () {
      ...
      Base.async(function () {
        ...
        document.addEventListener('keydown', tabSpy); // Simulate TAB.

        MockInteractions.pressAndReleaseKeyOn(document, 9);
        assert.equal(dom(overlay).querySelector('[autofocus]'), document.activeElement, 'focus stays on button');
        assert.isTrue(tabSpy.calledOnce, 'keydown spy called');
        assert.isTrue(tabSpy.getCall(0).args[0].defaultPrevented, 'keydown default prevented'); // Cleanup.
...
suite('focusable nodes', function () {
  ...
  test('with-backdrop: TAB & Shift+TAB wrap focus', function (done) {
    overlay.withBackdrop = true;
    runAfterOpen(overlay, function () {
      var focusableNodes = overlay._focusableNodes; // 1ms timeout needed by IE10 to have proper focus switching.

      Base.async(function () {
        // Go to last element.
        focusableNodes[focusableNodes.length - 1].focus(); // Spy keydown.
        ...
        assert.isTrue(tabSpy.calledOnce, 'keydown spy called');
        assert.isTrue(tabSpy.getCall(0).args[0].defaultPrevented, 'keydown default prevented'); // Simulate Shift+TAB.
        ...
        assert.isTrue(tabSpy.getCall(1).args[0].defaultPrevented, 'keydown default prevented again'); // Cleanup.
  ...
  test('with-backdrop: TAB & Shift+TAB wrap focus respecting tabindex', function (done) {
    ...
    runAfterOpen(overlayWithTabIndex, function () {
      var focusableNodes = overlayWithTabIndex._focusableNodes; // 1ms timeout needed by IE10 to have proper focus switching.

      Base.async(function () {
        // Go to last element.
        focusableNodes[focusableNodes.length - 1].focus(); // Simulate TAB.

        MockInteractions.pressAndReleaseKeyOn(document, 9);
        assert.equal(focusableNodes[0], document.activeElement, 'focus wrapped to first focusable'); // Simulate Shift+TAB.
  ...
  test('with-backdrop: Shift+TAB after open wrap focus', function (done) {
    ...
    runAfterOpen(overlay, function () {
      ...
      Base.async(function () {
        // Spy keydown.
        var tabSpy = sinon.spy();
        document.addEventListener('keydown', tabSpy); // Simulate Shift+TAB.
  ...
  test('with-backdrop: after open, update last focusable node and then Shift+TAB', function (done) {
    ...
    runAfterOpen(overlay, function () {
      ...
      Base.async(function () {
        ...
        overlay.invalidateTabbables(); // Simulate Shift+TAB.
...
suite('restore-focus-on-close', function () {
  ...
  test('does not return focus on close by default (restore-focus-on-close=false)', function (done) {
    ...
    runAfterOpen(overlay, function () {
      runAfterClose(overlay, function () {
        assert.notEqual(IronOverlayManager.deepActiveElement, focusable, 'focus is not restored to focusable');
        done();
      });
    });
  });
  test('overlay returns focus on close', function (done) {
    ...
    runAfterOpen(overlay, function () {
      runAfterClose(overlay, function () {
        assert.equal(IronOverlayManager.deepActiveElement, focusable, 'focus restored to focusable');
        done();
      });
    });
  });
  ...
  test('avoids restoring focus if focus changed', function (done) {
    ...
    runAfterOpen(overlay, function () {
      button1.focus();
      runAfterClose(overlay, function () {
        assert.equal(IronOverlayManager.deepActiveElement, button1, 'focus was not modified');
        done();
      });
    });
  });
...
suite('overlay with backdrop', function () {
  ...
  test('backdrop appears behind the overlay', function (done) {
    runAfterOpen(overlay, function () {
      ...
      var backdropStyleZ = parseInt(window.getComputedStyle(overlay.backdropElement).zIndex, 10);
  ...
  test('backdrop is removed when the element is removed from DOM', function (done) {
    runAfterOpen(overlay, function () {
      overlay.parentNode.removeChild(overlay); // Ensure detached is executed.
  ...
  test('manager.getBackdrops() updated on opened changes', function (done) {
    runAfterOpen(overlay, function () {
      assert.equal(IronOverlayManager.getBackdrops().length, 1, 'overlay added to manager backdrops');
      runAfterClose(overlay, function () {
        assert.equal(IronOverlayManager.getBackdrops().length, 0, 'overlay removed from manager backdrops');
        done();
      });
    });
  });
...
suite('multiple overlays', function () {
  ...
  test('close an overlay in proximity to another overlay', function (done) {
    ...
    overlay1.open();
    overlay1.close(); // Open the overlay we care about.

    overlay2.open(); // Immediately close the first overlay.
...
suite('overlay in composed tree', function () {
  ...
  test('with-backdrop wraps the focus within the overlay', function (done) {
    // 1ms timeout needed by IE10 to have proper focus switching.
    Base.async(function () {
      var buttons = dom(overlay).querySelectorAll('button'); // Go to last element.

      buttons[buttons.length - 1].focus(); // Spy keydown.
