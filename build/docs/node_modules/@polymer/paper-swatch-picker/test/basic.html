...
<body>
  ...
  <test-fixture id="preselected">
    <template>
      <paper-swatch-picker color="#ffcdd2"></paper-swatch-picker>
    </template>
  </test-fixture>
  ...
  <script type="module">import '../paper-swatch-picker.js';
...
suite('basic', function () {
  ...
  setup(function () {
    ...
    flush(); // This element has a dom-repeat.
  });
  test('clicking the button opens a rendered menu', function (done) {
    // No color is selected.
    assert.equal(picker.color, undefined);
    picker.$.iconButton.click();
    Base.async(function () {
      // There are 18 default columns, and each column is 20px wide.
      assert.equal(container.style.width, 18 * 20 + 'px'); // There are 10 default rows, and each row is 20px wide.

      assert.equal(container.style.height, 10 * 20 + 'px'); // The colors have been filled in. The first box is #ffebee.
...
suite('custom color list', function () {
  ...
  test('the picker shows the right colors', function (done) {
    ...
    Base.async(function () {
      ...
      assert.equal(container.style.width, 5 * 10 + 'px'); // There is 1 default row, and each row is 10px wide.

      assert.equal(container.style.height, 10 + 'px'); // The colors have been filled in correctly
...
suite('dynamic', function () {
  ...
  test('changing the number of columns updates the layout', function () {
    // There are 18 default columns, and each column is 20px wide.
    assert.equal(container.style.width, 18 * 20 + 'px'); // There is 10 default rows, and each row is 20px wide.

    assert.equal(container.style.height, 10 * 20 + 'px'); // Now change the number of columns from 18 to 9

    picker.columnCount = 9;
    assert.equal(picker.columnCount, 9); // Now we have 9 columns, and each column is 20px wide.

    assert.equal(container.style.width, 9 * 20 + 'px'); // Now we have 20 rows, and each column is 20px wide.
  ...
  test('changing the colors updates the colors and the layout', function (done) {
    ...
    Base.async(function () {
      ...
      Base.async(function () {
        ...
        picker.colorList = ['#ffcdd2', '#f44336', '#b71c1c', '#c5cae9', '#3f51b5', '#1a237e', '#b2dfdb', '#009688', '#004d40', '#fff9c4', '#ffeb3b', '#f57f17', '#d7ccc8', '#795548', '#3e2723', '#f5f5f5', '#9e9e9e', '#212121']; // Force the dom-repeat to re-render.

        flush();
        assert.equal(picker.colorList.length, 18); // Now we have 1 row, and each column is 20px wide.

        assert.equal(container.style.height, 20 + 'px'); // Since the old selected color is not in the new list,
