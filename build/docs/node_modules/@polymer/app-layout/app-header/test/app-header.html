...
import { flush as flush$0 } from "../../../polymer/lib/legacy/polymer.dom.js";
...
function scrollTestHelper(scroller, tests) {
  ...
  function triggerScrollEvent() {
    ...
    if (tests[0].y === scrollTop) {
      ...
      if (scroller === document.documentElement) window.scrollTo(0, tests[0].y);else scroller.scrollTop = tests[0].y;
    }
  }
...
function assertHeaderIsCondensed(header) {
  var headerClientRect = header.getBoundingClientRect();
  assert.equal(headerClientRect.top + headerClientRect.height, header._stickyEl.offsetHeight, 'condensed at ' + header._scrollTop);
}

function assertHeaderIsHidden(header) {
  var headerClientRect = header.getBoundingClientRect();
  assert.isBelow(headerClientRect.bottom, 0, 'hidden at ' + header._scrollTop);
}
