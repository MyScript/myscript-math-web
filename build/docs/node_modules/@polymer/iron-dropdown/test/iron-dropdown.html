...
<body>
  ...
  <script type="module">import '../iron-dropdown.js';
...
suite('<iron-dropdown>', function () {
  ...
  suite('locking scroll', function () {
    ...
    suiteSetup(function () {
      ...
      document.body.appendChild(bigDiv); // Need to discover if html or body is scrollable.
    ...
    test('should lock, only once', function (done) {
      var openCount = 0;
      runAfterOpen(dropdown, function () {
        expect(IronDropdownScrollManager._lockingElements.length).to.be.equal(1);
        expect(IronDropdownScrollManager.elementIsScrollLocked(document.body)).to.be.equal(true);
  ...
  suite('non locking scroll', function () {
    ...
    test('can be disabled with `allowOutsideScroll`', function (done) {
      runAfterOpen(dropdown, function () {
        expect(IronDropdownScrollManager.elementIsScrollLocked(document.body)).to.be.equal(false);
  ...
  suite('when align is left/top, with an offset', function () {
    ...
    test('can be offset towards the bottom right', function (done) {
      runAfterOpen(dropdown, function () {
        ...
        dropdown.horizontalOffset = 10; // Force refit instead of waiting for requestAnimationFrame.

        dropdown.refit();
        offsetDropdownRect = dropdown.getBoundingClientRect(); // verticalAlign is top, so a positive offset moves down.

        assert.equal(dropdownRect.top + 10, offsetDropdownRect.top, 'top ok'); // horizontalAlign is left, so a positive offset moves to the right.
    ...
    test('can be offset towards the top left', function (done) {
      runAfterOpen(dropdown, function () {
        ...
        dropdown.refit();
        offsetDropdownRect = dropdown.getBoundingClientRect(); // verticalAlign is top, so a negative offset moves up.

        assert.equal(dropdownRect.top - 10, offsetDropdownRect.top, 'top ok'); // horizontalAlign is left, so a negative offset moves to the left.
  ...
  suite('when align is right/bottom, with an offset', function () {
    ...
    test('can be offset towards the top left', function (done) {
      runAfterOpen(dropdown, function () {
        ...
        dropdown.refit();
        offsetDropdownRect = dropdown.getBoundingClientRect(); // verticalAlign is bottom, so a positive offset moves up.

        assert.equal(dropdownRect.bottom - 10, offsetDropdownRect.bottom, 'bottom ok'); // horizontalAlign is right, so a positive offset moves to the left.
    ...
    test('can be offset towards the bottom right', function (done) {
      runAfterOpen(dropdown, function () {
        ...
        dropdown.refit();
        offsetDropdownRect = dropdown.getBoundingClientRect(); // verticalAlign is bottom, so a negative offset moves down.

        assert.equal(dropdownRect.bottom + 10, offsetDropdownRect.bottom, 'bottom ok'); // horizontalAlign is right, so a positive offset moves to the right.
