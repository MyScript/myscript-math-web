...
<body>
  ...
  <script type="module">import '@polymer/test-fixture/test-fixture.js';
...
suite('multi', function () {
  ...
  test('set multi-selection via selected property', function () {
    // set selectedValues
    s.selectedValues = [0, 2]; // check selected class

    assert.isTrue(s.children[0].classList.contains('iron-selected'));
    assert.isTrue(s.children[2].classList.contains('iron-selected')); // check selectedItems
  ...
  test('set multi-selection via tap', function () {
    ...
    MockInteractions.tap(s.children[2]); // check selected class
  ...
  test('fire iron-select/deselect events when selectedValues changes', function () {
    ...
    s.selectedValues = [0, 1]; // programatically select values 1 and 2 (2 fires select, 0 fires deselect)

    s.selectedValues = [1, 2]; // programatically deselect all values (1 and 2 fire deselect)

    s.selectedValues = []; // check events
  ...
  test('fire iron-select/deselect events when selectedValues is modified', function () {
    ...
    s.selectedValues = []; // programatically select value 0

    s.push('selectedValues', 0, 1); // programatically deselect value 0

    s.shift('selectedValues'); // programatically select value 2

    s.push('selectedValues', 2); // programatically deselect value 1
  ...
  test('fire iron-select/deselect events when toggling items', function () {
    ...
    MockInteractions.tap(items[1]); // programatically select values 1 and 2 (2 fires select, 0 fires deselect)

    s.selectedValues = [1, 2]; // tap to deselect items 1 and 2 (both fire deselect)

    MockInteractions.tap(items[1]);
    MockInteractions.tap(items[2]); // check events
  ...
  test('toggle iron-selected class when toggling items selection', function () {
    ...
    var item0 = s.children[0],
        item1 = s.children[1];
    assert.isFalse(item0.classList.contains('iron-selected'));
    assert.isFalse(item1.classList.contains('iron-selected')); // tap to select item 0 (add iron-selected class)

    MockInteractions.tap(item0);
    assert.isTrue(item0.classList.contains('iron-selected'));
    assert.isFalse(item1.classList.contains('iron-selected')); // tap to select item 1 (add iron-selected class)

    MockInteractions.tap(item1);
    assert.isTrue(item0.classList.contains('iron-selected'));
    assert.isTrue(item1.classList.contains('iron-selected')); // tap to deselect item 1 (remove iron-selected class)

    MockInteractions.tap(item1);
    assert.isTrue(item0.classList.contains('iron-selected'));
    assert.isFalse(item1.classList.contains('iron-selected')); // programatically select both values (1 add iron-selected class)

    s.selectedValues = [0, 1];
    assert.isTrue(item0.classList.contains('iron-selected'));
    assert.isTrue(item1.classList.contains('iron-selected')); // programatically deselect all values (both removes iron-selected class)
  ...
  test('selects from items created by dom-repeat', function (done) {
    ...
    if (PolymerElement) {
      selector = fixture('repeatedItems-polymer2'); // `observeNodes` callbacks in Polymer 2.x are no longer called
    ...
    domRepeat.render(); // See the note in `forceSynchronousItemUpdate`.

    selector.forceSynchronousItemUpdate(); // NOTE(cdata): I guess `dom-repeat` doesn't stamp synchronously..
  ...
  test('updates selection when dom changes', function () {
    ...
    flush(); // See the note in `forceSynchronousItemUpdate`.
