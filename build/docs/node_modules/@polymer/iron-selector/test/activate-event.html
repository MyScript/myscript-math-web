...
suite('activate event', function () {
  ...
  test('activates on tap', function () {
    assert.equal(s.selected, '0'); // select Item 1

    s.children[1].dispatchEvent(new CustomEvent('tap', {
      bubbles: true
    }));
    assert.equal(s.selected, '1');
  });
  test('activates on tap and fires iron-activate', function (done) {
    assert.equal(s.selected, '0'); // attach iron-activate listener
  ...
  test('tap on already selected and fires iron-activate', function (done) {
    ...
    s.children[0].dispatchEvent(new CustomEvent('tap', {
      bubbles: true
    }));
  });
  test('activates on mousedown', function () {
    // set activateEvent to mousedown
    s.activateEvent = 'mousedown'; // select Item 2

    s.children[2].dispatchEvent(new CustomEvent('mousedown', {
      bubbles: true
    }));
  ...
  test('no activation', function () {
    assert.equal(s.selected, '0'); // set activateEvent to null

    s.activateEvent = null; // select Item 2
  ...
  test('activates on tap and preventDefault', function () {
    ...
    s.children[2].dispatchEvent(new CustomEvent('tap', {
      bubbles: true
  ...
  test('activates after detach and re-attach', function () {
    ...
    parent.appendChild(s); // select Item 2
