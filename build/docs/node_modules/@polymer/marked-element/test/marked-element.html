...
import { PolymerElement } from "../../polymer/polymer-element.js"; // Thanks IE10.
...
suite('<marked-element> with .markdown-html child', function () {
  ...
  suite('respects camelCased HTML', function () {
    ...
    test('in code blocks', function () {
      ...
      expect(outputElement).to.equal(markedElement.outputElement);
      expect(isHidden(markedElement.$.content)).to.be.true; // If Markdown content were put into a `<template>` or directly into the
  ...
  suite('respects bad HTML', function () {
    ...
    test('in code blocks', function () {
      ...
      var isEqualToOneOfThem = proofElement.innerHTML === '<p><div><p></p></div>' || proofElement.innerHTML === '<p></p><div><p></p></div>';
...
suite('<marked-element> without .markdown-html child', function () {
  suite('respects camelCased HTML', function () {
    ...
    test('in code blocks', function () {
      ...
      flush$0();
      expect(isHidden(markedElement.$.content)).to.be.false; // If Markdown content were put into a `<template>` or directly into the
      // DOM, it would be rendered as DOM and be converted from camelCase to
      // lowercase per HTML parsing rules. By using `<script>` descendants,
      // content is interpreted as plain text.
  ...
  suite('succesful fetch', function () {
    ...
    test('renders new remote content when src changes', function (done) {
      markedElement.addEventListener('marked-loadend', function firstCheck() {
        ...
        dom(markedElement).querySelector('[type="text/markdown"]').src = 'test2.md';
        markedElement.addEventListener('marked-loadend', function () {
          expect(outputElement.innerHTML).to.equal(proofElement.innerHTML);
          done();
        });
