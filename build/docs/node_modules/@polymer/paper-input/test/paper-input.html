...
<body>
  ...
  <script type="module">import '../paper-input.js';
...
function getFloatingLabel(paperInput) {
  return dom(dom(paperInput.root).querySelector('paper-input-container').root).querySelector('.label-is-floating');
}
...
suite('basic', function () {
  test('setting value sets the input value', function (done) {
    var input = fixture('basic'); // Mutation observer is async, so wait one tick.
  ...
  test('placeholder does not overlap label', function (done) {
    var input = fixture('placeholder'); // Mutation observer is async, so wait one tick.
  ...
  test('special types autofloat the label', function (done) {
    var input = fixture('date'); // Mutation observer is async, so wait one tick.
  ...
  test('setting the value to the empty string unfloats the label', function (done) {
    ...
    Base.async(function () {
      ...
      assert.equal(getFloatingLabel(input), null); // The label is floating if it has a value.

      input.value = 'foobar';
      assert.ok(getFloatingLabel(input)); // The label isn't floating if it gets reset.
  ...
  test('character counter is displayed', function (done) {
    var input = fixture('basic-char-counter'); // Need to wait a tick to stamp the char-counter.
  ...
  test('character counter is correct for type=number', function (done) {
    var input = fixture('type-number-char-counter'); // Need to wait a tick to stamp the char-counter.
  ...
  test('validator is used', function (done) {
    var input = fixture('validator'); // Mutation observer is async, so wait one tick.
...
suite('focus/blur events', function () {
  ...
  test('blur events fired on host element', function (done) {
    ...
    Base.async(function () {
      ...
      assert(input.focused, 'input is focused');
      assert(inputFocusSpy.callCount > 0, 'focus event fired');
      var blursBecauseOfFocus = blurFocusSpy.callCount; // We can't call blur() on the paper-input, because in shady dom
  ...
  test('focusing then bluring with shift-tab removes the focused attribute correctly', function (done) {
    ...
    Base.async(function () {
      input.focus();
      assert(input.focused, 'input is focused'); // Fake a shift-tab induced blur by forcing the flag.
...
suite('validation', function () {
  test('invalid attribute updated after calling validate()', function (done) {
    var input = fixture('required-no-auto-validate'); // Mutation observer is async, so wait one tick.

    Base.async(function () {
      input.validate(); // Need to wait a tick to stamp the error message.
...
suite('a11y', function () {
  test('has aria-labelledby, which is monotonically increasing', function (done) {
    var inputs = fixture('multiple-inputs'); // Mutation observer is async, so wait one tick.
  ...
  test('has aria-describedby for error message', function (done) {
    var input = fixture('required'); // Need to wait a tick to stamp the error message.
  ...
  test('has aria-describedby for character counter and error', function (done) {
    var input = fixture('required-char-counter'); // Need to wait a tick to stamp the char-counter and error message.

    Base.async(function () {
      assert.isTrue(getNativeInput(input).hasAttribute('aria-describedby'));
      var ariaDescribedBy = getNativeInput(input).getAttribute('aria-describedby');
      assert.notEqual(ariaDescribedBy.indexOf(getErrorDiv(input).id, -1, 'aria-describedby points to the error message'));
      assert.notEqual(ariaDescribedBy.indexOf(getCharCounterDiv(input).id, -1, 'aria-describedby points to the character counter'));
      done();
    }, 1);
  });
  test('focus an input with tabindex', function (done) {
    ...
    var input = fixture('has-tabindex'); // Mutation observer is async, so wait one tick.

    Base.async(function () {
      input.focus();
      assert.equal(input.shadowRoot ? input.shadowRoot.activeElement : document.activeElement, input._focusableElement);
      done();
    }, 1);
  });
});
