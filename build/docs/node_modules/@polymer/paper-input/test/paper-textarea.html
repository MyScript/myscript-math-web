...
<body>
  ...
  <script type="module">import '../paper-textarea.js';
...
suite('a11y', function () {
  ...
  test('has aria-describedby for character counter', function (done) {
    var input = fixture('char-counter'); // Need to wait a tick to stamp the char-counter.
  ...
  test('has aria-describedby for character counter and error', function (done) {
    ...
    Base.async(function () {
      ...
      assert.notEqual(ariaDescribedBy.indexOf(dom(input.root).querySelector('paper-input-error').id, -1, 'aria-describedby points to the error message'));
      assert.notEqual(ariaDescribedBy.indexOf(dom(input.root).querySelector('paper-input-char-counter').id, -1, 'aria-describedby points to the character counter'));
      done();
    }, 1);
  });
});</script>
