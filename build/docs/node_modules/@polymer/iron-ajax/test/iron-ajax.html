...
--><html><head>
  ...
  <script type="module" src="../iron-ajax.js"></script>
</head>
<body>
  ...
  <script type="module">import { Base } from "../../polymer/polymer-legacy.js";
...
suite('<iron-ajax>', function () {
  ...
  setup(function () {
    ...
    server.respondWith('GET', /\/responds_to_get_with_json.*/, [200, responseHeaders.json, '{"success":true}']);
    server.respondWith('POST', '/responds_to_post_with_json', [200, responseHeaders.json, '{"post_success":true}']);
    server.respondWith('GET', '/responds_to_get_with_text', [200, responseHeaders.plain, 'Hello World']);
    server.respondWith('GET', '/responds_to_debounced_get_with_json', [200, responseHeaders.json, '{"success": "true"}']);
    server.respondWith('GET', '/responds_to_get_with_502_error_json', [502, responseHeaders.json, '{"message": "an error has occurred"}']);
  ...
  suite('when url isn\'t set yet', function () {
    test('we don\'t fire any automatic requests', function () {
      ...
      return timePasses(1).then(function () {
        ...
        expect(server.requests.length).to.be.equal(0); // Explicitly asking for the request to fire works.

        ajax.generateRequest();
        expect(server.requests.length).to.be.equal(1);
        server.requests = []; // Explicitly setting url to '' works too.
  ...
  suite('when there are multiple requests', function () {
    ...
    test('avoids race conditions with last response', function () {
      expect(echoAjax.lastResponse).to.be.equal(undefined); // Resolving the oldest request doesn't update lastResponse.

      respondToEchoRequest(server.requests[0]);
      return requests[0].completes.then(function () {
        expect(echoAjax.lastResponse).to.be.equal(undefined); // Resolving the most recent request does!
    ...
    test('`loading` is true while the last one is loading', function () {
      ...
      return requests[0].completes.then(function () {
        ...
        expect(echoAjax.loading).to.be.eql(false); // Resolving an out of order stale request after should have
        // no effect.
  ...
  suite('when making POST requests', function () {
    ...
    test('if `contentType` is set to form encode, the body is encoded', function () {
      ...
      ajax.generateRequest();
      expect(server.requests[0]).to.be.ok;
      expect(server.requests[0].requestBody).to.be.equal('foo=bar%0D%0Abip&biz+bo=baz+blar');
    });
    test('if `contentType` is json, the body is json encoded', function () {
      ...
      ajax.generateRequest();
      expect(server.requests[0]).to.be.ok;
      expect(server.requests[0].requestBody).to.be.equal(JSON.stringify(requestObj));
    });
    ...
    suite('and `contentType` isn\'t set', function () {
      test('we don\'t try to encode an ArrayBuffer', function () {
        ...
        ajax.body = requestObj;
        ajax.generateRequest();
        expect(server.requests[0]).to.be.ok; // We give the browser the ArrayBuffer directly, without trying
        // to encode it.
  ...
  suite('when handleAs parameter is `text`', function () {
    test('response type is string', function () {
      ...
      expect(server.requests.length).to.be.equal(1);
      expect(server.requests[0].requestHeaders['accept']).to.be.equal('text/plain');
      server.respond();
      return promise;
    });
  });
  ...
  suite('when making a POST over the wire', function () {
    test('FormData is handled correctly', function () {
      ...
      return ajax.generateRequest().completes.then(function () {
        expect(ajax.lastResponse.headers['Content-Type']).to.match(/^multipart\/form-data; boundary=.*$/);
    ...
    test('json is handled correctly', function () {
      ...
      ajax.contentType = 'application/json';
      return ajax.generateRequest().completes.then(function () {
        expect(ajax.lastResponse.headers['Content-Type']).to.match(/^application\/json(;.*)?$/);
    ...
    test('urlencoded data is handled correctly', function () {
      ...
      return ajax.generateRequest().completes.then(function () {
        expect(ajax.lastResponse.headers['Content-Type']).to.match(/^application\/x-www-form-urlencoded(;.*)?$/);
        expect(ajax.lastResponse.form.a).to.be.equal('foo');
        expect(ajax.lastResponse.form.b).to.be.equal('bar');
      });
    });
    test('xml is handled correctly', function () {
      ...
      ajax.body = xmlDoc;
      return ajax.generateRequest().completes.then(function () {
        expect(ajax.lastResponse.headers['Content-Type']).to.match(/^application\/xml(;.*)?$/);
        expect(ajax.lastResponse.data).to.match(/<foo\s*><bar\s+name="baz"\s*\/><\/foo\s*>/);
      });
    });
  });
  ...
  suite('when using the bubbles attribute', function () {
    ...
    test('the request and error events should bubble to window', function (done) {
      ...
      window.addEventListener('iron-ajax-presend', incrementTotal);
      window.addEventListener('request', incrementTotal);
      window.addEventListener('iron-ajax-request', incrementTotal); // NOTE(cdata): This needs to be capturing because Mocha + Firefox
  ...
  suite('Progress Tests', function () {
    ...
    test('lastProgress is same as lastRequest', function (done) {
      ...
      ajax.lastRequest.completes.then(function () {
        ...
        ajax.removeEventListener('last-progress-changed', onLastProgressChanged);
        ajax.generateRequest(); // should reset
