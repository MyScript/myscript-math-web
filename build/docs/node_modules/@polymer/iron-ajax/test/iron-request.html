...
--><html><head>
  ...
  <script type="module" src="../iron-request.js"></script>
</head>
<body>
...
suite('<iron-request>', function () {
  ...
  setup(function () {
    ...
    server.respondWith('GET', '/responds_to_get_with_json', [200, jsonResponseHeaders, '{"success":true}']);
    server.respondWith('GET', '/responds_to_get_with_prefixed_json', [200, jsonResponseHeaders, '])}while(1);</x>{"success":true}']);
    server.respondWith('GET', '/responds_to_get_with_500', [500, {}, '']);
    server.respondWith('GET', '/responds_to_get_with_100', [100, {}, '']);
    server.respondWith('GET', '/responds_to_get_with_0', [0, jsonResponseHeaders, '{"success":true}']);
  ...
  suite('basic usage', function () {
    ...
    test('headers are sent up', function () {
      ...
      expect(fakeXhr.requestHeaders['accept']).to.be.equal('this should override the default');
    });
    test('headers are deduped by lowercasing', function () {
      ...
      expect(fakeXhr.requestHeaders['foo']).to.be.equal('bar');
      expect(fakeXhr.requestHeaders['accept']).to.be.equal('this should also override the default');
    });
  });
