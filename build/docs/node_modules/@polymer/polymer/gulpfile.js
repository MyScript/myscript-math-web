...
class BackfillStream extends Transform {
  ...
  _transform(file, enc, cb) {
    if (this.fileList) {
      const origFile = this.fileList.shift(); // console.log(`rename ${file.path} -> ${origFile.path}`)
...
class AddClosureTypeImport extends Transform {
  ...
  _transform(file, enc, cb) {
    if (file.path === this.target) {
      ...
      let html = parse5.parse(contents, {
        locationInfo: true
      });
...
gulp.task('closure', gulp.series('generate-externs', () => {
  ...
  const project = new PolymerProject({
    shell: `./${entry}`,
    fragments: ['node_modules/@webcomponents/shadycss/entrypoints/apply-shim.js', 'node_modules/@webcomponents/shadycss/entrypoints/custom-style-interface.js'],
    extraDependencies: [addClosureTypes.importPath, 'externs/polymer-internal-shared-types.js']
  });

  function closureLintLogger(log) {
    let chalk = require('chalk'); // write out log to use with diffing tools later
  ...
  const closureStream = closure({
    ...
    polymer_version: 2,
    externs: ['bower_components/shadycss/externs/shadycss-externs.js', 'externs/webcomponents-externs.js', 'externs/closure-types.js', 'externs/polymer-externs.js'],
    extra_annotation_name: ['appliesMixin', 'mixinClass', 'mixinFunction', 'polymer', 'customElement']
  }, closurePluginOptions);
  const closurePipeline = lazypipe().pipe(() => closureStream).pipe(() => new BackfillStream(closureStream.fileList_)); // process source files in the project

  const sources = project.sources(); // process dependencies

  const dependencies = project.dependencies(); // merge the source and dependencies streams to we can analyze the project

  const mergedFiles = mergeStream(sources, dependencies);
  const splitter = new HtmlSplitter();
  return mergedFiles.pipe(addClosureTypes).pipe(project.bundler()).pipe(splitter.split()).pipe(gulpif(splitRx, closurePipeline())).pipe(splitter.rejoin()).pipe(gulpif(joinRx, minimalDocument())).pipe(gulpif(joinRx, size({
    title: 'closure size',
    gzip: true,
    showTotal: false,
    showFiles: true
...
gulp.task('estimate-size', gulp.series('clean', () => {
  ...
  const bundlePipe = lazypipe().pipe(() => bundledSplitter.split()).pipe(() => gulpif(/\.js$/, babel(babelPresets))).pipe(() => bundledSplitter.rejoin()).pipe(minimalDocument);
  return mergedFiles.pipe(project.bundler()).pipe(gulpif(/polymer\.html$/, bundlePipe())).pipe(gulpif(/polymer\.html$/, size({
    title: 'bundled size',
    gzip: true,
    showTotal: false,
    showFiles: true
...
gulp.task('lint-eslint', function () {
  return gulp.src(['lib/**/*.js', 'test/unit/*.{html,js}', 'util/*.js']).pipe(eslint()).pipe(eslint.format()).pipe(eslint.failAfterError());
});
gulp.task('lint', done => {
...
gulp.task('update-version', () => {
  return gulp.src('lib/utils/boot.js').pipe(replace(/(window.Polymer.version = )'\d+\.\d+\.\d+'/, `$1'${require('./package.json').version}'`)).pipe(gulp.dest('lib/utils'));
});