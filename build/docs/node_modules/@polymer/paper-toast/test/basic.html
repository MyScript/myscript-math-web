...
--><html><head>
  ...
  <script type="module" src="../paper-toast.js"></script>
...
<body>
  ...
  <script type="module">import '../paper-toast.js';
suite('basic', function () {
  ...
  test('auto-close is correctly reset', function (done) {
    toast = fixture('basic');
    toast.duration = 10;
    toast.show(); // a bit later (before the auto-close), toast is reset

    setTimeout(function () {
      toast.hide(); // keep toast opened
  ...
  test('toast is positioned according at the bottom left of its fitInto', function (done) {
    ...
    toast.open(); // Wait for it to be opened, so it will be sized correctly.

    toast.addEventListener('iron-overlay-opened', function () {
      var rect = toast.getBoundingClientRect();
      assert.equal(rect.left, 50, 'left ok'); // 150px from top, (100px of height + 50px of margin-top)
