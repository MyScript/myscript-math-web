...
<body>
  ...
  <script type="module">import '../iron-input.js';
...
suite('basic', function () {
  test('can initialize programmatically', function (done) {
    ...
    dom(ironInput).appendChild(document.createElement('input')); // Mutation observer is async, so wait one tick.

    Base.async(function () {
      var input = ironInput.querySelector('input');
      assert.ok(input);
      assert.ok(ironInput.inputElement); // Updating bind-value updates the input's value.

      ironInput.bindValue = 'foobar';
      assert.equal(input.value, ironInput.bindValue, 'input value equals to iron-input bindValue'); // Updating the input's value updates bind-value;
  ...
  test('setting bindValue sets value', function (done) {
    var ironInput = fixture('basic'); // Mutation observer is async, so wait one tick.
  ...
  test('default bindValue sets value', function (done) {
    var ironInput = fixture('has-bind-value'); // Mutation observer is async, so wait one tick.
  ...
  test('set bindValue to undefined', function () {
    ...
    var templ = document.getElementById('bind-to-object-template'); // The data lives in a different place in Polymer 1.0 vs 2.0.
  ...
  test('prevent invalid input works after an input event', function (done) {
    var ironInput = fixture('prevent-invalid-input'); // Mutation observer is async, so wait one tick.
  ...
  test('inputs can be validated', function (done) {
    var ironInput = fixture('basic-validation'); // Mutation observer is async, so wait one tick.
  ...
  test('validator used instead of constraints api if provided', function (done) {
    var ironInput = fixture('has-validator')[1]; // Mutation observer is async, so wait one tick.
  ...
  test('browser validation beats custom validation', function (done) {
    var ironInput = fixture('native-and-custom-validator')[1]; // Mutation observer is async, so wait one tick.

    Base.async(function () {
      var input = ironInput.querySelector('input'); // The input allows any letters, but the browser only allows one
      // of [abc].

      input.value = 'aaaa';
      fireNativeInputEvent(input);
      ironInput.validate();
      assert.isFalse(ironInput.invalid, 'input is valid'); // The validator allows this, but the browser doesn't.
  ...
  test('prevent invalid input works automatically when allowed pattern is set', function (done) {
    var ironInput = fixture('automatically-prevent-invalid-input-if-allowed-pattern'); // Mutation observer is async, so wait one tick.
