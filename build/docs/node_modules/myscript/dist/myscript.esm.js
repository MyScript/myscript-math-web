    ...
    return function (t) {
      ...
      var r = "function" == typeof Symbol && "symbol" == typeof Symbol.iterator ? function (t) {
        ...
        return t && "function" == typeof Symbol && t.constructor === Symbol && t !== Symbol.prototype ? "symbol" : typeof t;
          ...
          i = function t(e) {
        var n = this;
        o(this, t), this.toJSON = function (t) {
          if ("string" != typeof t) return console.error("Need a CSS string but given ", void 0 === t ? "undefined" : r(t), t), "Not a valid CSS..!";
          var e = {},
              o = void 0,
              i = void 0,
              u = void 0;
          ...
          try {
            t.split("{").forEach(function (t) {
              if (i = t.trim()) if (-1 === i.indexOf("}")) e[i] = {}, o = i;else {
                i.substring(0, i.indexOf("}")).split(";").forEach(function (t) {
                  (u = t.split(":")) && 2 === u.length && (e[o][u[0].trim().replace(/^\"|\"$/g, "")] = n._trimSemiColon(u[1].trim().replace(/^\"|\"$/g, "")));
                });

                try {
                  o = i.split("}")[1].trim(), o && (e[o] = {});
                } catch (t) {}
              }
            });
          } catch (t) {
          ...
          return e;
        }, this.toCSS = function (t) {
          if ("object" !== (void 0 === t ? "undefined" : r(t))) return console.error("Need a JSON object but given ", void 0 === t ? "undefined" : r(t), t), "Not a valid JSON..!";
          var e = "";
          ...
          return e;
        }, this._trimSemiColon = function (t) {
          return ";" === t.slice(-1) ? t.slice(0, n.length - 1) : t;
        };
      };
...
function attach(element, editor) {
  ...
  function pointerUpHandler(evt) {
    ...
    var scrollbarClasses = ['ps__rail-x', 'ps__thumb-x']; // Check if pointer entered into any smartguide elements or scrollbar
    ...
    var pointerEnteredSmartGuide = evt.relatedTarget && (smartGuideIds.includes(evt.relatedTarget.className) || scrollbarClasses.includes(evt.relatedTarget.className)); // Check if pointer didn't stay in the smartguide and pointer exited the smartguide or scrollbar
    ...
    var pointerExitedSmartGuide = evt.relatedTarget && evt.target && (smartGuideIds.includes(evt.target.className) || scrollbarClasses.includes(evt.target.className)); // Check if pointer moved between words in smartguide
...
function drawEllipseArc(context, centerPoint, maxRadius, minRadius, orientation, startAngle, sweepAngle) {
  ...
  try {
    ...
    for (var i = 0; i <= n; i++) {
      ...
      var sinAlpha = Math.sin(alpha); // current point
...
var MusicClefs = {
  // constants was calculated from the svg image, so it should remains together
  F: {
...
function slice(stroke) {
  ...
  var slicedStroke = createStrokeComponent({
    color: stroke.color,
    width: stroke.width
  });
...
function getDefaultMusicSymbols(configuration) {
  var defaultStaff = Object.assign({}, {
    type: 'staff'
  }, configuration.recognitionParams.v3.musicParameter.staff);
  var defaultClef = {
    type: 'clef',
...
function initPendingStroke(model, point, properties) {
  ...
  if (properties && properties['-myscript-pen-width']) {
    ...
    Object.assign(properties, {
      width: pxWidth / 2
    }); // FIXME hack to get better render
  }

  var modelReference = model;
  modelLogger.trace('initPendingStroke', point); // Setting the current stroke to an empty one
...
function endPendingStroke(model, point, penStyle) {
  ...
  if (modelReference.currentStroke) {
    ...
    var currentStroke = addPoint(modelReference.currentStroke, point); // Mutating pending strokes

    addStroke(modelReference, currentStroke);
    addStrokeToGroup(modelReference, currentStroke, penStyle); // Resetting the current stroke to an undefined one
...
function getBorderCoordinates(model) {
  ...
  if (model.recognizedSymbols && model.recognizedSymbols.length > 0) {
    modelBounds = getSymbolsBounds(model.recognizedSymbols, modelBounds); // Pending strokes
...
function cloneModel(model) {
  var clonedModel = Object.assign({}, model); // We clone the properties that need to be. Take care of arrays.
...
function getPixelRatio(canvas) {
  if (canvas) {
    var context = canvas.getContext('2d'); // we are using a browser object
    // eslint-disable-next-line no-undef
...
function detectPixelRatio(element) {
  ...
  var canvasRatio = getPixelRatio(tempCanvas); // document.removeChild(tempCanvas);
...
function drawModel(context, model, stroker) {
  context.renderingCanvasContext.clearRect(0, 0, context.renderingCanvas.width, context.renderingCanvas.height); // Displaying the default symbols and pending strokes

  var symbols = [].concat(toConsumableArray(model.defaultSymbols)); // Displaying the recognition symbols or raw strokes
...
function drawStroke$1(context, stroke) {
  ...
  try {
    ...
    if (length < 3) {
      ...
      renderLine(contextReference, firstPoint, computeMiddlePoint(firstPoint, getPointByIndex(stroke, 1)), width); // Possibility to try this (the start looks better when the ink is large)
...
function drawCurrentStroke$1(context, model, stroker) {
  var modelRef = model; // Add a pending id for pending strokes rendering

  modelRef.currentStroke.id = 'pendingStroke-' + model.rawStrokes.length; // Render the current stroke
...
function xhr(type, url, data) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    request.onerror = function () {
      reject({
        msg: 'Could not connect to ' + url + ' connection error',
        recoverable: false
      });
    };
...
function buildInput$2(recognizerContext, model) {
  ...
  Object.assign(input, {
    parameter: configuration.recognitionParams.v3.analyzerParameter
  }); // Building the input with the suitable parameters
...
var Cdkv3RestAnalyzerRecognizer =
/*#__PURE__*/
Object.freeze({
...
function buildWebSocketCallback(destructuredPromise, recognizerContext) {
  return function (message) {
    var recognizerContextRef = recognizerContext; // Handle websocket messages
    ...
    switch (message.type) {
        ...
        switch (message.data.type) {
          ...
          case 'error':
            ...
            if (recognitionContext) {
              ...
              destructuredPromise.reject(Object.assign({}, message.data, {
                recoverable: false
              }));
            }
      ...
      case 'error':
        ...
        if (recognitionContext) {
          recognitionContext.callback(Object.assign({}, message, {
            recoverable: false
          }));
        } else {
          destructuredPromise.reject(Object.assign({}, message, {
            recoverable: false
          }));
        }
...
function buildConfiguration(configuration) {
  return Object.assign({
    type: 'configuration'
  }, configuration.recognitionParams.v4);
}
...
var Cdkv4WSInteractiveRecognizer =
/*#__PURE__*/
Object.freeze({
  init: init$5,
...
function eventCallback(type, data) {
  callbackLogger.info('emitting ' + type + ' event', data); // We are making usage of a browser provided class
  // eslint-disable-next-line no-undef

  this.dispatchEvent(new CustomEvent(type, Object.assign({
    bubbles: true,
    composed: true
  }, data ? {
    detail: data
...
function computeStats(model) {
  ...
  if (model.rawStrokes) {
    ...
    var restMessage = buildInput(createEmptyRecognizerContext({
    ...
    stats.pointsCount = model.rawStrokes.map(function (stroke) {
      ...
      return a + b;
    }, 0); // We start with 270 as it is the size in bytes. Make a real computation implies to recode a doRecognition
...
function createCanvas$1(borderCoordinates) {
  var margin = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 10; // eslint-disable-next-line no-undef
...
PerfectScrollbar.prototype.update = function update() {
  ...
  this.railXMarginWidth = toInt(get$2(this.scrollbarXRail).marginLeft) + toInt(get$2(this.scrollbarXRail).marginRight);
  this.railYMarginHeight = toInt(get$2(this.scrollbarYRail).marginTop) + toInt(get$2(this.scrollbarYRail).marginBottom); // Hide scrollbars not to affect scrollWidth and scrollHeight
...
function clickCandidate(evt, editor, smartGuide) {
  ...
  if (candidate !== smartGuideRef.wordToChange.label && smartGuideRef.wordToChange.candidates.includes(candidate)) {
    ...
    jiixToImport.words[smartGuideRef.wordToChange.id].label = candidate; // eslint-disable-next-line no-underscore-dangle
...
function callFadeOutObserver() {
  ...
  var smartGuideRef = smartGuide;
  var elementsRef = smartGuide.elements; // eslint-disable-next-line no-undef
  ...
  observer.observe(elementsRef.smartGuideElement, {
    childList: true,
    subtree: true,
    attributes: true
  });
...
function createSmartGuide(editor) {
  ...
  var perfectScrollbar = new PerfectScrollbar(elements.textContainer, {
...
function resize$3(smartGuide) {
  ...
  var maxWidthTextContainer = smartGuideRef.editor.domElement.clientWidth - left - elementsRef.tagElement.offsetWidth - 35 - left; // Assign a max width to the smartguide based on the editor width, the left position and a small margin for the ellipsis (48px)
...
function insertSmartGuide(smartGuide) {
  ...
  var insertTextContainer = function insertTextContainer(left, maxWidth) {
    elementsRef.textContainer.style.left = left + 'px'; // Assign a max width to the smartguide based on the editor width, the left position and a small margin for the ellipsis (48px)
  ...
  var marginLeft = smartGuideRef.editor.configuration.recognitionParams.v4.text.margin.left * mmToPixels; // 12 is the space between line in mm

  var top = marginTop - 12 * mmToPixels;
  var left = marginLeft;
  insertSmartGuideElement(left, top);
  insertTag(); // 35 is the ellipsis element width
  ...
  elementsRef.candidatesElement.style.display = 'none';
  elementsRef.smartGuideElement.appendChild(elementsRef.candidatesElement); // 48px as set in css
...
function launchSmartGuide(smartGuide, exports) {
  ...
  var populatePrompter = function populatePrompter(words) {
    elementsRef.textElement.innerHTML = ''; // We use a DocumentFragment to reflow the DOM only one time as it is not part of the DOM
    ...
    words.forEach(function (word, index) {
      if (word.label === ' ' || word.label.includes('\n')) {
        ...
        var span = createWordSpan(false, index, word); // This is used to scroll to last word if last word is modified
  ...
  if (exports && JSON.parse(exports[Constants.Exports.JIIX]).words.length > 0) {
    ...
    smartGuideRef.previousLabelExport = JSON.parse(exports[Constants.Exports.JIIX]).label; // This is required by clipboard.js to get the text to be copied.
...
function recognizerCallback(editor, error, model) {
  ...
  var handleResult = function handleResult(err, res) {
    ...
    if (err) {
      ...
      if ( // IInk error managment before refactor
      err.message === 'Invalid application key.' || err.message === 'Invalid HMAC' || // CDK error managment
      err.error && err.error.result && err.error.result.error && (err.error.result.error === 'InvalidApplicationKeyException' || err.error.result.error === 'InvalidHMACSignatureException') || // IInk error managment after refactor
...
var Editor = function () {
  ...
  function Editor(element, configuration, penStyle, theme, behaviors) {
    ...
    this.domElement.classList.add('ms-editor'); // eslint-disable-next-line no-undef

    this.loader = document.createElement('div');
    this.loader.classList.add('loader');
    this.loader = this.domElement.appendChild(this.loader); // eslint-disable-next-line no-undef
  ...
  createClass(Editor, [{
    ...
    value: function pointerDown(point) {
      ...
      this.renderer.drawCurrentStroke(this.rendererContext, this.model, this.stroker); // Currently no recognition on pointer down
    }
    /**
    ...
    value: function pointerMove(point) {
      ...
      this.renderer.drawCurrentStroke(this.rendererContext, this.model, this.stroker); // Currently no recognition on pointer move
    }
    /**
     * Handle a pointer up
     * @param {{x: Number, y: Number, t: Number}} point Captured point coordinates
    ...
    value: function import_(data, mimetype) {
      ...
      launchImport(this, this.model, !(data instanceof Blob) ? new Blob([data], {
        type: mimetype
      }) : data);
    }
    /**
    ...
    set: function set$$1(recognizer) {
      ...
      var initialize = function initialize(model) {
        ...
        if (_this5.innerRecognizer) {
           ...
           */
          _this5.recognizerContext = createEmptyRecognizerContext(_this5); // FIXME: merge undo/redo manager with default recognizer
      ...
      if (recognizer) {
        if (this.innerRecognizer) {
           ...
           * @type {Model}
           */
          this.model = createModel(this.configuration); // INFO: Recognizer needs model to be initialized
...
export { Constants, defaultConfiguration as DefaultConfiguration, defaultBehaviors as DefaultBehaviors, defaultPenStyle as DefaultPenStyle, defaultTheme as DefaultTheme, register, getAvailableLanguageList, log as LoggerConfig, Editor, InkModel, RecognizerContext }; //# sourceMappingURL=myscript.esm.js.map