...
defineFunction(["\\left", "\\right"], {
  ...
  const delim = checkDelimiter(args[0], context); // \left and \right are caught somewhere in Parser.js, which is
...
defineFunction(["\\acute", "\\grave", "\\ddot", "\\tilde", "\\bar", "\\breve", "\\check", "\\hat", "\\vec", "\\dot", "\\widehat", "\\widetilde", "\\overrightarrow", "\\overleftarrow", "\\Overrightarrow", "\\overleftrightarrow", "\\overgroup", "\\overlinesegment", "\\overleftharpoon", "\\overrightharpoon"], {
  ...
  const base = args[0];
  const isStretchy = !utils.contains(["\\acute", "\\grave", "\\ddot", "\\tilde", "\\bar", "\\breve", "\\check", "\\hat", "\\vec", "\\dot"], context.funcName);
  const isShifty = !isStretchy || utils.contains(["\\widehat", "\\widetilde"], context.funcName);
  return {
    type: "accent",
