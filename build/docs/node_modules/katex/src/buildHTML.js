...
import buildCommon, { makeSpan } from "./buildCommon.js";
import delimiter from "./delimiter.js";
import domTree from "./domTree.js";
import units from "./units.js";
import utils from "./utils.js";
import stretchy from "./stretchy.js";
...
const buildExpression = function (expression, options, isRealGroup) {
  ...
  for (let i = 0; i < groups.length; i++) {
    if (groups[i].value === "\u0338" && i + 1 < groups.length) {
      ...
      const container = makeSpan(classes, children); // LaTeX adds a space between ords separated by a \\not.
...
const shouldHandleSupSub = function (group, options) {
  if (!group.value.base) {
    ...
    if (base.type === "op") {
      ...
      return base.value.limits && (options.style.size === Style.DISPLAY.size || base.value.alwaysHandleSupSub);
...
groupTypes.color = function (group, options) {
  const elements = buildExpression(group.value.value, options.withColor(group.value.color), false); // \color isn't supposed to affect the type of the elements it contains.
...
groupTypes.supsub = function (group, options) {
  ...
  const scriptspace = 0.5 / metrics.ptPerEm / multiplier + "em";
  let supsub;
...
groupTypes.array = function (group, options) {
  ...
  let nc = 0;
  let body = new Array(nr); // Horizontal spacing
  ...
  const jot = 3 * pt; // Default \arraystretch from lttab.dtx
  ...
  c < nc || colDescrNum < colDescriptions.length; ++c, ++colDescrNum) {
    ...
    while (colDescr.type === "separator") {
      ...
      if (colDescr.separator === "|") {
        const separator = makeSpan(["vertical-separator"], []);
...
groupTypes.llap = function (group, options) {
  const inner = makeSpan(["inner"], [buildGroup(group.value.body, options)]);
  const fix = makeSpan(["fix"], []);
  return makeSpan(["mord", "llap"], [inner, fix], options);
};
...
groupTypes.op = function (group, options) {
  ...
  if (hasLimits) {
    ...
    let subm;
    let subKern;
    let newOptions; // We manually have to handle the superscripts and subscripts. This,
...
groupTypes.mod = function (group, options) {
  ...
  if (group.value.modType !== "pod") {
    const modInner = [buildCommon.mathsym("m", group.mode), buildCommon.mathsym("o", group.mode), buildCommon.mathsym("d", group.mode)];
...
groupTypes.katex = function (group, options) {
  ...
  const k = makeSpan(["k"], [buildCommon.mathsym("K", group.mode)], options);
  const a = makeSpan(["a"], [buildCommon.mathsym("A", group.mode)], options);
  a.height = (a.height + 0.2) * 0.75;
  a.depth = (a.height - 0.2) * 0.75;
  const t = makeSpan(["t"], [buildCommon.mathsym("T", group.mode)], options);
  const e = makeSpan(["e"], [buildCommon.mathsym("E", group.mode)], options);
  e.height = e.height - 0.2155;
  e.depth = e.depth + 0.2155;
  const x = makeSpan(["x"], [buildCommon.mathsym("X", group.mode)], options);
...
groupTypes.overline = function (group, options) {
  ...
  const innerGroup = buildGroup(group.value.body, options.havingCrampedStyle()); // Create the line above the body
...
groupTypes.underline = function (group, options) {
  ...
  const innerGroup = buildGroup(group.value.body, options); // Create the line above the body
...
groupTypes.sqrt = function (group, options) {
  ...
  // and line
  let inner = buildGroup(group.value.body, options.havingCrampedStyle()); // Some groups can return document fragments.  Handle those by wrapping
  // them in a span.
  ...
  const minDelimiterHeight = (inner.height + inner.depth + lineClearance + theta) * options.sizeMultiplier; // Create a sqrt SVG of the required minimum size

  const img = delimiter.customSizedDelim("\\surd", minDelimiterHeight, false, options, group.mode); // Calculate the actual line width.
  // This actually should depend on the chosen font -- e.g. \boldmath
  // should use the thicker surd symbols from e.g. KaTeX_Main-Bold, and
  // have thicker rules.

  const ruleWidth = options.fontMetrics().sqrtRuleThickness * img.sizeMultiplier;
  ...
  const imgShift = img.height - inner.height - lineClearance - ruleWidth; // We add a special case here, because even when `inner` is empty, we
  ...
  if (!group.value.index) {
    ...
    const rootm = buildGroup(group.value.index, newOptions, options); // The amount the index is shifted by. This is taken from the TeX
  ...
  if (hadMiddle) {
    for (let i = 1; i < inner.length; i++) {
      ...
      if (middleDelim.isMiddle) {
        ...
        inner[i] = delimiter.leftRightDelim(middleDelim.isMiddle.value, innerHeight, innerDepth, middleDelim.isMiddle.options, group.mode, []); // Add back spaces shifted into the delimiter
...
groupTypes.rule = function (group, options) {
  ...
  const height = units.calculateSize(group.value.height, options); // Style the rule to the right size
...
groupTypes.accent = function (group, options) {
  ...
  if (group.type === "supsub") {
    ...
    const supsub = group; // The real accent group is the base of the supsub group
  ...
  const mustShift = group.value.isShifty && isCharacterBox(base); // Calculate the skew of the accent. This is based on the line "If the
  ...
  const clearance = Math.min(body.height, options.fontMetrics().xHeight); // Build the accent

  let accentBody;

  if (!group.value.isStretchy) {
    const accent = buildCommon.makeSymbol(group.value.label, "Main-Regular", group.mode, options); // Remove the italic correction of the accent, because it only serves to
...
groupTypes.horizBrace = function (group, options) {
  ...
  const body = buildGroup(group.value.base, options.havingBaseStyle(Style.DISPLAY)); // Create the stretchy element
  ...
  if (hasSupSub) {
    ...
    //   ┏━━━━━━━━┓   or    ┏━━━┓     not    ┏━━━━━━━━━┓
    //    equation           eqn                 eqn
    const vSpan = makeSpan(["mord", group.value.isOver ? "mover" : "munder"], [vlist], options);
...
groupTypes.accentUnder = function (group, options) {
  ...
  const kern = /tilde/.test(group.value.label) ? 0.12 : 0; // Generate the vlist, with the appropriate kerns
...
groupTypes.enclose = function (group, options) {
  ...
  if (label === "sout") {
    ...
    const isCharBox = isCharacterBox(group.value.body); // ref: LaTeX source2e: \fboxsep = 3pt;  \fboxrule = .4pt
...
groupTypes.xArrow = function (group, options) {
  ...
  const upperShift = -options.fontMetrics().axisHeight - arrowBody.height - 0.111; // 2 mu. Ref: amsmath.dtx: #7\if0#2\else\mkern#2mu\fi
  // Generate the vlist

  let vlist;

  if (group.value.below) {
    const lowerShift = -options.fontMetrics().axisHeight + lowerGroup.height + arrowBody.height + 0.111;
    vlist = buildCommon.makeVList([{
      type: "elem",
      elem: upperGroup,
      shift: upperShift
...
groupTypes.phantom = function (group, options) {
  const elements = buildExpression(group.value.value, options.withPhantom(), false); // \phantom isn't supposed to affect the elements it contains.
...
const buildGroup = function (group, options, baseOptions) {
  ...
  if (groupTypes[group.type]) {
    ...
    let groupNode = groupTypes[group.type](group, options); // If the size changed between the parent and the current group, account
...
const buildHTML = function (tree, options) {
  ...
  const body = makeSpan(["base"], expression, options); // Add struts, which ensure that the top of the HTML element falls at the
