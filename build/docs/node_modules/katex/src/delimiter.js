...
const makeLargeDelim = function (delim, size, center, options, mode, classes) {
  ...
  const span = styleWrap(makeSpan(["delimsizing", "size" + size], [inner], options), Style.TEXT, options, classes);
...
const makeInner = function (symbol, font, mode) {
  ...
  const inner = makeSpan(["delimsizinginner", sizeClass], [makeSpan([], [buildCommon.makeSymbol(symbol, font, mode)])]); // Since this will be passed into `makeVList` in the end, wrap the element
...
const makeStackedDelim = function (delim, heightTotal, center, options, mode, classes) {
  ...
  return styleWrap(makeSpan(["delimsizing", "mult"], [inner], newOptions), Style.TEXT, options, classes);
};
...
const sqrtSpan = function (height, delim, options) {
  ...
  if (delim.type === "small") {
    ...
    const vertSegment = viewBoxHeight - 54; // This \sqrt is customized in both height and width. We set the
