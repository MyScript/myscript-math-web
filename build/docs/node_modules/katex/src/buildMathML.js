 ...
 * parser.
 */
import buildCommon, { makeSpan, fontMap } from "./buildCommon.js";
import fontMetrics from "./fontMetrics.js";
import mathMLTree from "./mathMLTree.js";
...
groupTypes.text = function (group, options) {
  const body = group.value.body; // Convert each element of the body into MathML, and combine consecutive
...
groupTypes.genfrac = function (group, options) {
  const node = new mathMLTree.MathNode("mfrac", [buildGroup(group.value.numer, options), buildGroup(group.value.denom, options)]);
  ...
  if (group.value.leftDelim != null || group.value.rightDelim != null) {
    ...
    if (group.value.leftDelim != null) {
      const leftOp = new mathMLTree.MathNode("mo", [new mathMLTree.TextNode(group.value.leftDelim)]);
    ...
    if (group.value.rightDelim != null) {
      const rightOp = new mathMLTree.MathNode("mo", [new mathMLTree.TextNode(group.value.rightDelim)]);
...
groupTypes.leftright = function (group, options) {
  ...
  if (group.value.left !== ".") {
    const leftNode = new mathMLTree.MathNode("mo", [makeText(group.value.left, group.mode)]);
    leftNode.setAttribute("fence", "true");
    inner.unshift(leftNode);
  }

  if (group.value.right !== ".") {
    const rightNode = new mathMLTree.MathNode("mo", [makeText(group.value.right, group.mode)]);
...
groupTypes.middle = function (group, options) {
  const middleNode = new mathMLTree.MathNode("mo", [makeText(group.value.middle, group.mode)]);
...
groupTypes.accent = function (group, options) {
  ...
  if (group.value.isStretchy) {
    ...
    accentNode = new mathMLTree.MathNode("mo", [makeText(group.value.label, group.mode)]);
  }

  const node = new mathMLTree.MathNode("mover", [buildGroup(group.value.base, options), accentNode]);
...
groupTypes.katex = function (group) {
  const node = new mathMLTree.MathNode("mtext", [new mathMLTree.TextNode("KaTeX")]);
  return node;
};
...
groupTypes.overline = function (group, options) {
  ...
  const node = new mathMLTree.MathNode("mover", [buildGroup(group.value.body, options), operator]);
  node.setAttribute("accent", "true");
  return node;
};
...
groupTypes.underline = function (group, options) {
  const operator = new mathMLTree.MathNode("mo", [new mathMLTree.TextNode("\u203e")]);
  operator.setAttribute("stretchy", "true");
  const node = new mathMLTree.MathNode("munder", [buildGroup(group.value.body, options), operator]);
...
groupTypes.accentUnder = function (group, options) {
  ...
  const node = new mathMLTree.MathNode("munder", [buildGroup(group.value.body, options), accentNode]);
  node.setAttribute("accentunder", "true");
  return node;
};

groupTypes.enclose = function (group, options) {
  const node = new mathMLTree.MathNode("menclose", [buildGroup(group.value.body, options)]);
  let notation = "";
...
groupTypes.horizBrace = function (group, options) {
  const accentNode = stretchy.mathMLnode(group.value.label);
  return new mathMLTree.MathNode(group.value.isOver ? "mover" : "munder", [buildGroup(group.value.base, options), accentNode]);
};
...
groupTypes.llap = function (group, options) {
  const node = new mathMLTree.MathNode("mpadded", [buildGroup(group.value.body, options)]);
...
const buildMathML = function (tree, texExpression, options) {
  const expression = buildExpression(tree, options); // Wrap up the expression in an mrow so it is presented in the semantics
  // tag correctly.

  const wrapper = new mathMLTree.MathNode("mrow", expression); // Build a TeX annotation of the source
